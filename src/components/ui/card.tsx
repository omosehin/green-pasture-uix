...
// Ported unmodified from ogaryde-admin-ui/src/components/ui/card.tsx — a
...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "rounded-lg border bg-card text-card-foreground shadow-sm transition-[border-color,box-shadow] duration-200 ease-out hover:border-ring/40 hover:shadow-md motion-reduce:transition-none",
        className
      )}
      {...props}
    />
